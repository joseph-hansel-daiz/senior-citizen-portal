...
interface CreateSeniorResponse {
  id: number;
  barangayId: number;
  isDeleted: boolean;
  createdAt: string;
...
export function useCreateSenior() {
  ...
  const [data, setData] = useState<CreateSeniorResponse | null>(null);

  const createSenior = async (payload: CreateSeniorPayload) => {
    setLoading(true);
    setError(null);
    setData(null);

    try {
      const response = await fetch("http://localhost:8000/seniors", {
        method: "POST",
        headers: {
